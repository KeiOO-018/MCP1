...
일곱 건이 났다. **여섯은 PIE나 되읽기로 잡았고, 하나는 원인을 못 찾은 채 재실행으로 넘어갔다.**

**1. 명령 9가 통째로 안 먹었다.** 상단 벽 `11`개의 트랜스폼을 고치라고 냈는데 되읽으니 `11`개 전부 이전 값 그대로였고 액터 수도 안 늘었다. 즉 만들지도 고치지도 않았다. **원인은 못 찾았다.** 사용자가 같은 명령을 다시 돌리자 통했다. 명령문도 대상도 바꾼 게 없다. **기대 바운드를 명령과 같이 낸 덕에 "일부 실패"가 아니라 "통째로 안 돎"이라는 것을 즉시 알았다** — 표와 대조만 하면 됐다.

**2. 벽 바깥 모서리에 `100 × 100` 노치가 12군데 생겼다.** 사용자가 스크린샷으로 `벽이 끝에서 한칸씩 안쪽으로 땡겨져서 생성됬는데 이거 의도한거야?`라고 물었다. 지시대로 정확히 배치됐는데도 생긴 것이라 **배치 오류가 아니라 설계의 결과**였다.

원인은 상단 벽이 실내 쪽으로 `100` 물러난 것이다. 수직으로 만나는 두 상단 벽이 각각 물러나므로 **바깥 모서리의 `100 × 100` 정사각형을 아무도 안 덮는다.** `Z 200..400` 구간이다. 이 구조는 `Lvl_ThirdPerson`에서 그대로 옮겨온 것이고 원본에도 같은 노치가 있었을 것이다.

고친 방법은 **상단 벽을 하단과 같은 두께 `200`으로 올려 같은 면으로 맞춘 것**이다(명령 9). 대안이었던 "모서리에 `100×100×200` 큐브 12개 채우기"는 액터가 `12`개 느는 데 비해 턱이 남아서 버렸다. **덤으로 하나가 더 고쳐졌다** — 문간이 하단 벽 두께 `200`을 관통하는데 상단은 `100`만 덮고 있어서 **문간 바깥쪽 `100` 구간에 `Z 200..400` 높이의 틈**이 나 있었다. 문 셋 전부 그랬고 같이 막혔다.

**3. 문짝이 문간보다 위아래로 `10`씩 삐져나왔다.** 사용자가 열린 문 스크린샷으로 `문이 벽 사이 공간보다 조금 더 높다`고 짚었다.

원인은 `DoorMesh`의 `RelativeScale3D.z = 1.1`이다. 액터 스케일과 곱해 월드에서 문짝 높이가 `220`인데 문간은 `200`이라 `Z -10..210`을 차지했다. **폭은 `200`으로 문간과 정확히 맞춰놨는데 높이만 `1.1`인 것**이 근거가 되어, 틈을 막으려고 일부러 키운 게 아니라 흘려둔 값으로 판단했다. **닫혀 있을 때는 안 보인다** — 문이 벽 두께 한가운데에 있어 넘치는 `10`이 바닥과 인방 속에 묻힌다. 열 때만 드러났다.

고친 방법은 템플릿을 `1.0`으로 내린 것인데(명령 14) **인스턴스가 안 따라왔다.** 아래 4번과 같은 문제라 결국 **문 셋을 지우고 다시 만들어** 해결했다. 새로 스폰한 액터는 바뀐 템플릿 값을 물고 나온다.

**4. `set_properties`와 `reset_properties`가 `true`를 반환하면서 안 썼다.** 3번을 인스턴스에서 고치려다 걸렸다. 터미널이 같은 프로퍼티에 `z: 2.0`, `y: 0.25`까지 넣어보고 전부 안 먹는 것을 확인했고, **모든 호출이 `true`를 반환했다.** `reset_properties`는 요청하지도 않은 `X` 멤버를 `1`로 바꿔놓아 값을 오히려 망가뜨렸다.

원인은 모른다. **경계만 좁혔다** — 액터 변수 쓰기(`bLocked`, `MyDoor`, `RoomExtent`)는 되고, BP 템플릿 컴포넌트 쓰기(`DoorMesh_GEN_VARIABLE`)도 되고, **배치 인스턴스의 컴포넌트 트랜스폼만** 안 된다.

우회로가 둘 확인됐다. **액터를 지우고 다시 만들거나**(문 셋에 썼다), **값을 변수로 받아 `ConstructionScript`가 컴포넌트에 밀어 넣거나**(`RoomBounds`에 썼다). 변수는 인스턴스에 쓸 수 있으므로 후자는 항상 통한다.

**5. PIE 오류 40여 줄과 부활한 적이 안 움직인 것이 같은 원인이었다.** 사용자가 `내가 죽고나서 다시스폰하고 (...) 다시 들어가면, 적 AI가 나를 인지 못함`이라고 보고했고, 로그에 `Accessed None trying to read (real) property CallFunc_GetController_ReturnValue_5`가 매 `Tick` 두 줄씩 쌓여 있었다. **`Node: Branch`와 `Node: StopMovement`라는 위치 정보가 붙어 있어서 `GetController`가 `None`이라는 것이 바로 읽혔다.**

가설은 "부활한 적에게 AI 컨트롤러가 없다"였고, **틀렸다면 `AutoPossessAI`가 이미 `PlacedInWorldOrSpawned`일 것**이라고 반증 조건을 먼저 적었다. 읽어보니 `PlacedInWorld`였다. **레벨에 배치된 폰만 컨트롤러를 받고 `SpawnActor`로 만든 폰은 못 받는다.** 컨트롤러가 없으면 `SimpleMoveToActor`도 `StopMovement`도 전부 무효라 적이 제자리에 서 있는다.

CDO를 `PlacedInWorldOrSpawned`로 바꿔 고쳤다. **배치된 적 여섯의 인스턴스 값은 `PlacedInWorld`로 남아 있는데** 배치된 폰은 그 값으로도 컨트롤러를 받으므로 무해하고, 부활한 적은 `SpawnActor`가 CDO를 읽으므로 바뀐 값을 받는다.

**6. 적이 뒤에서 때려도 안 돌아봤다.** 사용자가 `적 뒤에 있으면 얘가 날 보고 때리는게 아니라 그 상태에서 공격을 함`이라고 보고했다. 어제 기록에 접어둔 항목으로 남아 있던 것이다.

공격 체인을 읽으니 `Branch → StopMovement → PlayMontage`로 **회전 노드가 하나도 없었다.** `bUseControllerRotationYaw`도 `false`이고 `bOrientRotationToMovement`는 움직일 때만 돈다.

`StopMovement`와 `PlayMontage` 사이에 `SetActorRotation`을 하나 넣어 고쳤다. **`StopMovement` 뒤라서 이동 중 회전과 안 싸우고, 몽타주가 시작되기 전에 방향이 정해진다** — 몽타주 중간에 돌리면 공격 모션이 미끄러진다.

**7. 스테이지에 들어가도 문이 안 닫혔다.** 사용자가 `스테이지 입장하고 나면 문이 닫혀야 되는데 안닫히고 열려있던데`라고 보고했다.

**PIE 로그에 오류도 경고도 한 줄 없었다.** 그래서 예외가 아니라 분기가 안 타는 것으로 보고, 추측 대신 **정적으로 확인할 수 있는 것부터** 봤다 — 배치된 방에 `RoomBounds`가 실제로 있는지, 플레이어 캡슐이 그 박스와 겹칠 수 있는 설정인지.

원인이 거기서 나왔다. **배치된 방 셋의 `RoomBounds`가 `RelativeLocation (0,0,0)` · `BoxExtent (32,32,32)`, 즉 엔진 기본값이었다.** 템플릿은 `(0,0,200)` · `(700,800,200)`인데 안 내려간 것이고, 4번과 같은 인스턴스 동결이다. **방 정중앙 바닥에 `64³`짜리 상자가 놓여 있던 셈이다.**

이게 앞서 사용자가 `문 닫히는 속도가 너무 느리고`라고 한 것도 설명해준다 — 그때는 플레이어가 우연히 방 정중앙을 밟고 지나갔고, **거의 안 닫히는 것이 정상 동작이었다.**

고친 방법은 `RoomExtent`(Vector, Instance Editable)를 만들고 `ConstructionScript`가 `SetBoxExtent(RoomBounds, RoomExtent)`와 `SetRelativeLocation(RoomBounds, (0,0,RoomExtent.Z))`를 하게 한 것이다. **`RoomExtent` 자체도 인스턴스에서 `(0,0,0)`으로 굳어 있어서** 세 인스턴스에 직접 써넣어야 했다 — CS가 돈 증거가 박스가 `(32,32,32)`에서 `(0,0,0)`으로 바뀐 것이었다.

**덤으로 제약이 하나 풀렸다.** 그 전에는 "컴포넌트 프로퍼티를 인스턴스에 못 쓰니 방마다 다른 크기를 줄 수 없다"고 적었는데, 변수를 거치면 방마다 다르게 줄 수 있다. `Room1`은 세로 `1600`이라 `(700,800,200)`, `Room2`·`Room3`은 `1800`이라 `(700,900,200)`이다.

**`Z` 위치를 `RoomExtent.Z`에서 뽑은 것**도 의도적이다. 박스 반높이와 중심이 변수 하나로 같이 움직여서 어긋날 수 없다.

**문 여닫기 속도는 문제가 아니라 판단 번복이었다.** `SwingSpeed`를 `1 → 3`(`1.00초 → 0.33초`)으로 올렸다가 사용자가 `열릴 떄 너무 빠른거 같아서`라고 해서 `1`로 되돌렸다. **애초에 "닫는 게 느리다"는 판단이 7번의 트리거가 고장 나 있을 때 나온 것**이라, 박스를 고친 지금은 문 바로 앞에서 닫히므로 같은 `1`초라도 느낌이 다르다. 되돌리는 쪽이 공짜라 먼저 되돌렸다.

**진단에서 반복된 것 셋.** 첫째, **로그를 먼저 읽었다** — 5번은 오류 원문의 `Node:` 필드가 원인을 바로 가리켰고, 7번은 오류가 **없다는 것**이 "예외가 아니라 분기"라는 방향을 정했다. 둘째, **가설과 반증 조건을 먼저 적었다** — 5번에서 "틀렸다면 `AutoPossessAI`가 이미 `PlacedInWorldOrSpawned`일 것"이라고 써두고 읽었다. 셋째, **되읽기를 도구 반환값이 아니라 상태로 했다** — 1·3·4·7이 전부 `true`를 받고도 안 된 경우였다.
