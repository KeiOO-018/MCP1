...
- **없음.** 문 사양(`Docs/Spec/2026-08-30-문과-상호작용.md`)의 명령 30~41이 전부 끝났고 합격 기준 넷이 통과했다. 다음 작업은 사용자가 고른다.
  - 사용자가 정한 전체 순서는 `인벤토리 → 아이템 구조화·설계 → 아이템 배치·상호작용 → 적 AI → 전투`(1단계 사양)이고, **세 번째가 방금 끝났다.** 다음은 적 AI다
  - 다만 아래 `접어둔 것`의 첫 항목(카메라 작업)이 그 앞에 있다고 지난 세션이 정해뒀다

**결정 필요**

- **합격 기준 3의 경첩 반전을 다시 볼 것인가.** `bHingeOnRight`를 체크하고 액터 `Y`를 `-50` → `+50`으로 옮긴 상태에서 정상 개폐까지 확인할지. 지금은 문짝이 반대쪽으로 간다는 것까지만 확실하다. 되돌려놨으므로 다시 하려면 두 곳을 다시 고쳐야 한다
...
- **컴파일 경고.** 명령 39·41 둘 다 `compile_blueprint`가 `null`만 줬고 에디터의 Message Log를 안 읽었다. 다음에 블루프린트를 건드릴 때 한 번 열어보면 된다
- **`__ExternalObjects__`에 생긴 파일의 정체.** `DoorTest` 폴더의 `UActorFolder`로 **추정**한 채 커밋했다. `get_asset_class`가 외부 패키지를 못 읽는다
- **벽에 붙어 서서 `Q`.** 벽에 반쯤 묻힌 아이템이 나올 것으로 예상만 했다. 이제 진짜 벽이 레벨에 있으므로 바로 해볼 수 있다
- **같은 자리에 두 번 `Q`.** 쌓일 것으로 예상만 했다
- **경사면에 `Q`.** 스폰 회전이 `0,0,0`이라 안 눕는다. 얼마나 어색한지 안 봤다. `SM_Ramp11`이 `X 500~900`에 있다
...
- **원인 미상의 재직렬화 두 건.** `BP_Door`(커밋 `b4ab97a`)와 `BP_DoorFrame`(커밋 `8e8e30a`). 내용은 안 깨졌다. 다시 나오면 그때 본다. **명령 39에서 세 번째가 나왔다** — 스케일 값 하나를 바꿨는데 `BP_Door.uasset`이 804바이트 줄었다
- **`SetbOpen` 노드가 저절로 움직인 것.** 배선과 핀 값이 동일해서 동작에 영향이 없다
- **`Key`의 나머지 용도.** 이 사양이 문 하나를 여는 것까지만 했다. 스테이지별 열쇠, 여러 문, 진행 조건은 진행 구조 단계다
...
- **문틀 메시와 인방.** 지금 벽이 회색 박스 둘이고 문 위가 뚫려 있다. 맵을 만들 때 온다
- **`MoveComponentTo`의 `Stop`·`Return` 핀.** 연타로는 문제가 안 났다. 핀을 채운 것과 같지 않다
